...
router.get("/:id", async (req, res) => {
    ...
    try {
      ...
      if (user) return res.status(200).json({ id: user.id, email: user.email, username: user.username });
    } catch (error) {
    ...
    try {
        ...
        if (userExists) {
            ...
            if (passwordIsValid) {
                ...
                res.status(200).send({ token: token, user: { id: userExists.id, email: userExists.email, username: userExists.username } });
            }
